...
def run_streed(parameters, num_extra_cols):
    ...
    for key, value in parameters.items():
        if key == "core-file" or key == 'test-file' or key == 'hyper-tune' or key == 'max-depth' or key == 'max-num-nodes':
            continue
        args.append(f"-{key}")
        args.append(f"{value}")
...
def main():
    ...
    try:
        ...
        for params in params_settings:
            ...
            if (train_filename != 'train/Dialysis_partition_0'):
                continue
    ...
    # Write trees to file
    f = open(f"{DIRECTORY}/output/coxmodel_trees.csv", "w")
